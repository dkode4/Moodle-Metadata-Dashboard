...
target_ip = input("Enter IP to search: ").strip()
...
with open(INPUT_FILE, newline='', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        if len(row) > 0:
            ...
            ip = row[8].strip() if len(row) > 8 else 'Unknown'
            date_obj = parse_date(time_str)
            if date_obj and match_period(date_obj) and ip == target_ip:
                if user:
                    users.add(user)
...
if users:
    print("Users who performed actions from this IP:")
    for user in sorted(users):
        print(f"  {user}")
else:
    print("No users found for this IP in the selected period.")